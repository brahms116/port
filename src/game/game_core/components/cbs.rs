...
pub struct StateMotionCb<State>(
    pub fn(&mut State, &mut Motion, &mut Transform),
);

pub struct UpdateStateCb<State>(pub fn(&mut State));

pub struct CollisionCb<T: StaticCollisionMarker, K>(
    pub fn(Entity, &World, &Vec2),
    std::marker::PhantomData<T>,
    std::marker::PhantomData<K>,
);

impl<T: StaticCollisionMarker, K> CollisionCb<T, K> {
    pub fn new(func: fn(Entity, &World, &Vec2)) -> Self {
        Self(
            func,
